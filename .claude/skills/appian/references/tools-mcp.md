...
- **On interfaces with grids, charts, or several cards, the rendered component tree easily runs 50,000+ characters.** Guessing a "generous" `maxResponseSizeBytes` (e.g. 10000) still overflows and just wastes a call — if you only need `diagnostics.error`, not the rendered tree, pass a small value (e.g. `100`) on the *first* call so the response is written straight to a file, then read only the diagnostics with `jq '.diagnostics' <path>` instead of paging through the full tree.
