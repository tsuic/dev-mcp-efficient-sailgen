const FORBIDDEN_RULES = [
    ...
    // Non-existent components are now caught by param-name-validator.ts via
    // registry-based existence checks. No need for individual regex rules here.
];
/**
...
export function checkForbiddenPatterns(lines, isRecordMode) {
    const errors = [];
    lines.forEach((line, lineIndex) => {
        ...
        for (const rule of FORBIDDEN_RULES) {
            ...
            if (isRecordMode && RECORD_MODE_EXEMPT_IDS.has(rule.id))
                continue;
 ...
 *   )
 */
function checkWherecontainsScalarUnwrap(lines) {
    const errors = [];
    ...
    while ((match = wcRegex.exec(source)) !== null) {
        ...
        if (afterMatch)
            continue; // properly unwrapped — not a violation
        ...
        if (!isPositionArgumentOfIndex(source, match.index))
            continue;
...
function findMatchingCloseParen(source, openParenIdx) {
    let depth = 0;
    let inString = false;
 ...
 */
function isPositionArgumentOfIndex(source, wcStartIdx) {
    let depth = 0;
    let inString = false;
 ...
 */
function checkFvVariableContext(lines) {
    const errors = [];
    const source = lines.join("\n");
    ...
    function contextNamesAt(offset) {
        let result = [];
