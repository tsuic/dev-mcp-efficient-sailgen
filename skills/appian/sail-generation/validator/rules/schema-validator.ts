...
export function checkSchemaEnums(lines: string[]): ValidationError[] {
  ...
  lines.forEach((line, lineIndex) => {
    ...
    while ((match = PARAM_VALUE.exec(line)) !== null) {
      ...
      if (paramValue.includes("!") || paramValue.includes("(")) continue;
      // Skip TODO placeholders
      if (paramValue.startsWith("TODO") || paramValue === "PLACEHOLDER") continue;
      ...
      if (!allowedValues) continue; // parameter not in our enum table — skip

      // Check if hex is acceptable for this param
      if (HEX_ACCEPTING_PARAMS.has(paramName) && isHexColor(paramValue)) continue;
 ...
 */
function buildContextIndex(lines: string[]): (lineIndex: number, col: number) => string | undefined {
  ...
  const events: Array<{ offset: number; top: string | undefined }> = [];
  ...
  lines.forEach((line) => {
    for (let i = 0; i < line.length; i++) {
      ...
      if (!inString && ch === "/" && next === "*") { inComment = true; i++; absoluteOffset += 2; continue; }
      if (inComment && ch === "*" && next === "/") { inComment = false; i++; absoluteOffset += 2; continue; }
      if (inComment) { absoluteOffset++; continue; }
      if (ch === '"') {
        if (!inString) inString = true;
        else if (next === '"') { i++; absoluteOffset++; } // escaped quote
        else inString = false;
        absoluteOffset++;
        continue;
      }
      if (inString) { absoluteOffset++; continue; }
  ...
  return function contextAt(lineIndex: number, col: number): string | undefined {
    ...
    for (const ev of events) {
      if (ev.offset > targetOffset) break;
      result = ev.top;
    }
    return result;
  };
