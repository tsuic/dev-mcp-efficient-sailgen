 ...
 */
function checkUndeclaredLocals(
  source: string,
  openParenIdx: number,
  topLevelArgs: Arg[],
): ValidationError[] {
  const errors: ValidationError[] = [];
  if (topLevelArgs.length < 2) return errors; // need at least 1 decl + 1 body
  ...
  if (/a!localVariables\s*\(/.test(body)) return errors;

  const refPattern = /\blocal!([a-zA-Z_][a-zA-Z0-9_]*)\b/g;
  let match: RegExpExecArray | null;
  const reported = new Set<string>(); // report each undeclared name only once
