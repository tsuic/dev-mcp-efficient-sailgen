...
import { parseComponentTree, flattenTree, type ComponentNode } from "./lib/component-tree.js";
...
const CONTAINMENT: Record<string, { allowed: Set<string>; message: string }> = {
  "a!richTextDisplayField": {
    ...
    message:
      "a!richTextDisplayField's value/label may only contain richText children " +
...
// ── 2. REQUIRED PARAMS ───────────────────────────────────────────────────────
const REQUIRED_PARAMS: Record<string, { params: string[]; message: (missing: string) => string }> = {
  "a!gridColumn": {
    params: ["label", "value"],
...
const COMPANION_PARAMS: Array<{
...
}> = [
  {
    ...
    message:
      'a!gridField row selection needs both "selectionValue" and "selectionSaveInto" — ' +
  ...
  },
  {
    ...
    group: ["selectionValue", "selectionSaveInto"],
    message:
      'a!gridLayout row selection needs both "selectionValue" and "selectionSaveInto".',
  },
  {
    component: "a!cardLayout",
    group: ["decorativeBarColor", "decorativeBarPosition"],
    message:
      'a!cardLayout\'s decorative bar needs both "decorativeBarColor" and ' +
      '"decorativeBarPosition" — one without the other renders nothing.',
  },
];

export function checkStructuralRules(source: string): ValidationError[] {
...
function checkContainment(node: ComponentNode, errors: ValidationError[]): void {
  ...
  for (const child of node.children) {
    ...
    if (!KNOWN_COMPONENTS.has(child.name)) continue;
...
function checkRequiredParams(node: ComponentNode, errors: ValidationError[]): void {
...
function checkCompanionParams(node: ComponentNode, errors: ValidationError[]): void {
  for (const rule of COMPANION_PARAMS) {
    if (rule.component !== node.name) continue;
...
// ── NUMERIC RANGES ────────────────────────────────────────────────────────────
// Regex-based (the component tree doesn't track param values). Each rule matches
// `paramName: <number>` with a numeric literal only — expressions like
// `percentage: local!x` or `weight: fv!index` are skipped (no literal to check).
interface RangeRule {
  param: RegExp;
  min?: number;
  max?: number;
...
function checkNumericRanges(source: string): ValidationError[] {
  const errors: ValidationError[] = [];
...
function checkPageFrameInArray(source: string): ValidationError[] {
  const errors: ValidationError[] = [];
  const lines = source.split("\n");
  ...
  for (let i = 0; i < source.length; i++) {
    ...
    if (!inString && ch === "/" && next === "*") { inBlockComment = true; clean.push(" ", " "); i++; continue; }
    if (inBlockComment && ch === "*" && next === "/") { inBlockComment = false; clean.push(" ", " "); i++; continue; }
    if (inBlockComment) { clean.push(" "); continue; }
    if (ch === '"') {
      if (!inString) { inString = true; clean.push(" "); continue; }
      else if (next === '"') { clean.push(" ", " "); i++; continue; }
      else { inString = false; clean.push(" "); continue; }
    }
    if (inString) { clean.push(" "); continue; }
    clean.push(ch);
  }
  ...
  while ((m = PAGE_FRAME_RE.exec(cleanSrc)) !== null) {
    ...
    let prev = offset - 1;
    while (prev >= 0 && /[\s\n\r]/.test(cleanSrc[prev])) prev--;
    if (prev >= 0 && cleanSrc[prev] === "{") {
      // Determine line number
      let lineNum = 1;
      for (let c = 0; c < offset; c++) { if (source[c] === "\n") lineNum++; }
      errors.push({
