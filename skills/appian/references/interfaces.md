...
- Card galleries or repeated card layouts over data (`a!forEach` producing `a!cardLayout` per item)
- Dynamic grids where columns themselves are generated by iteration
- Interactive selection patterns with `local!` state (click-to-select, toggle highlighting)
...
**Note:** Tabular grids → use `RecordGrid`. Editable row grids → use `EditableGrid`.
These macros handle `a!forEach` internally. Only use raw SAIL when you need
card/tile repetition, conditional per-item styling, or click-to-select behavior.

### scaffoldInterface Example (record-bound form):

**Retry rule:** If `scaffoldInterface` fails twice, fall back to `createInterface` immediately.
Do not iterate beyond 2 attempts — the fast pivot produces better results than prolonged retries.
