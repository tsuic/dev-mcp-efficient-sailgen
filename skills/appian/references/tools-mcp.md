...
- Card galleries / repeated card layouts (`a!forEach` producing `a!cardLayout` per record)
- Dynamic grids where columns are generated by iteration
- Interactive selection with `local!` state (click-to-select, toggle highlighting)
- Complex `if()`/`choose()` conditional rendering
- Custom plugin components
- Patterns outside the component set above

**Clarification:** "I need to iterate over records" does NOT automatically mean raw SAIL.
- Tabular record list → use `RecordGrid` (scaffold handles it)
- Editable rows → use `EditableGrid` (scaffold handles it)
- Card/tile gallery, conditional per-item styling, click interactions → raw SAIL
